...
const examples = [
  ...
  row("bash", { command: "npm test" }, result([
    "> pi-codex-appearance test", "Running unit tests...", "fixture 1", "fixture 2", "fixture 3", "fixture 4", "fixture 5", "fixture 6",
  ...
  row("bash", { command: "npm run check" }, result("Checking TypeScript..."), { isPartial: true }),
  row("bash", { command: "cat /protected/config.json" }, result("cat: /protected/config.json: Permission denied\nCommand exited with code 1"), { isError: true }),
];
...
const html = `<!doctype html><html lang="zh-CN"><meta charset="utf-8"><title>Codex appearance 0.3 — renderer snapshot</title>
...
<div class="label">pi-codex-appearance 0.3.0 · GENERATED FORMATTER/LAYOUT SNAPSHOT · NOT A LIVE PI SESSION</div>
<pre>${ansiHtml(transcript)}</pre><div class="note">The edit block uses the same width-aware diff layout function as the Pi runtime adapter: exact Codex dark surfaces, full-row fill and hanging wraps.</div></html>`;
